...
> 全部完成，已归档。Phase 1-5 实施完毕，验收标准全部通过。
...
3. ✅ 定时扫描集成到后端启动流程（无下载器时自动启动 60s 轮询）
4. ✅ 前端下载管理面板适配无下载器模式（显示引导提示）
...
6. ✅ 前端设置页 qB/Alist 区域根据插件状态动态显隐
7. ✅ 下载管理面板根据已安装后端动态显示（无后端时显示引导）

### Phase 5：设置页 + 前端全面动态化 ✅

1. ✅ 设置页只显示 Core 配置 + 已安装插件配置（FIELD_GROUPS 按插件状态过滤）
2. ✅ 下载管理面板"从下载器同步"按钮根据 hasDownload 显隐
