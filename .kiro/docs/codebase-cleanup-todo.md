...
> **状态：路由拆分+测试归档+脚本清理+插件自加载改造已全部完成。前端拆分暂缓（风险高收益低）。Phase D 暂缓。**
...
| 660 | `routes/relocate.py` | ✅ **已完成**。树构建逻辑（_build_old_tree/_build_new_tree/_build_plan_tree，约 400 行）下沉到 `relocate_tree_builder.py`，relocate.py 保留路由（232 行） |
| 634 | `routes/organize.py` | 暂不拆。核心编排逻辑已通过 organize_executor/renamer/structure_organizer 做了业务层分离，路由层剩余 578 行是编排+参数处理 |
...
- [x] **Phase A**：灰色源文件从 git 跟踪中移除（.gitignore 排除，本地保留运行）
- [x] **Phase B**：插件自加载改造 — 让 plugins/ 下的插件通过 register(ctx) 自行加载源代码
- [x] **Phase C**：彻底删除根目录旧文件（bt_scraper_*.py / pan_scraper_*.py / rss_source_*.py）
- [ ] **Phase D**：Provider 框架文件移入 `core/`（暂缓 — 大规模 import 修改风险高，收益低）
- [x] **Phase E**：每个插件添加 `manifest.json`（已完成，所有插件目录已有 manifest.json）

> 详细改造计划见 `plugin-ecosystem-todo.md` Phase 2c
