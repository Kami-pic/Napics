...
- 不改搜索评分、过滤、排序。
- 不删除 `shared.py` 中旧 scraper getter；当前只允许由 `bt_search_provider_factory.py` 作为兼容桥集中引用。

## 收口结论

- `routes/search.py`、`search_service.py`、`search_helpers.py` 不再直接 import 具体 BT 直搜 scraper 或 `shared.py` getter。
- BT 直搜执行入口已统一经 `DirectBTSearchProviderAdapter.search()` 调用，再转换回旧 `SearchResult` 供现有 enrich / 去重 / SSE 响应复用。
- 具体 BT 源名称仍保留在配置、metadata、搜索词分发、测试、旧 scraper 实现和兼容工厂中；这些属于当前阶段允许边界。
- Prowlarr 仍保留旧客户端路径，按计划留到 Phase 6 单独迁移。

## 验证

- `cd backend && python -X utf8 -m pytest test_search_helpers_provider_bridge.py test_search_route_snapshots.py test_search_service_provider_bridge.py test_bt_search_provider_adapter.py test_bt_search_provider_factory.py test_provider_contracts.py test_provider_registry.py test_searcher.py`
