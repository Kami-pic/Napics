...
## 2026-05-26 Bug 修复 + 插件生态重新分层

**变更**:
- **Bug 修复 6 项**：
  1. 自动重命名功能修正：按 V3 设计，"自动命名"改为"生成标准名"，只做 shadow_only（影子名生成），不改物理文件名。移除预览→确认两步流程
  2. 扫描时清洗名生成：对所有新扫描结果都生成完整结构化清洗名（cn/en/original），不再只对空值补全
  3. "自动识别"文案改为"自动刮削"；CandidatePicker 默认 tab 根据 `default_scrape_source` 配置动态排序
  4. 季完整度显示恢复：ExpandPanel 的 tv 季卡片网格中显示每季完整度状态（完整✓/部分 local/total/缺失）
  5. 路径更换后清理旧数据：scan_path 保存时清理不属于任何已配置路径的孤立条目
  6. VideoDetail 的"自动重命名"同步改为"生成标准名"（shadow_only=true）

- **插件生态三层重新分层**：
  - **预装 5 个**：metadata-tmdb、metadata-douban、download-qbittorrent、feature-discover、feature-local-match
  - **大号仓库不预装 5 个**：metadata-bangumi、feature-completeness、feature-subscribe、search-prowlarr、storage-openlist
  - **小号仓库 11 个**（从 2 个大包拆为 11 个独立分组）：
    - BT 搜索 5 包：search-bt-mirror / search-bt-movie-tv / search-bt-anime-jp / search-bt-anime-cn / search-bt-cn
    - 网盘搜索 3 包：search-pan-main / search-pan-github / search-pan-resource
    - 其他 3 个：rss-anime / rss-tv-movie / download-openlist

- **小号仓库同步**：`icatmiumiu/plugins-of-napics` 推送 v3.0.0（index.json + 11 个插件目录含源文件）

**决策**:
- 插件拆分原则：按风险特征分组，一个源挂了只影响同组，不全军覆没
- BT 拆分维度：镜像站风险 / 影视公开站 / 日本动画 / 中文动画 / 中文磁力
- 网盘拆分维度：聚合引擎 / 同源数据（gogopanso+github）/ 资源站直搜
- `default_scrape_source` 默认改为 `"douban"`（豆瓣预装，中文用户优先）
- 物理重命名只在"一键整理"流程中执行，独立的"生成标准名"按钮只写影子名

---

## 2026-05-25 拆分回归排查 + 端到端测试

**变更**:
- **Bug 修复 5 个**：
  1. `routes/scrape.py`：`ShadowNameRequest` 类未定义 → 添加类定义（影子名设置端点 422）
  2. `frontend/ExpandPanel.tsx`：从 `./CardGrid` 导入造成循环依赖 → 改为从 `./cardGridUtils` 导入
  3. `routes/library.py`：`clean_from_filename(fn, source="parsed")` 参数错误 → 去掉 source 参数
  4. `tests/test_bt_expand.py`：引用已移除的 `shared.py` getter → 更新为新接口
  5. `tests/test_bt_search_provider_factory.py`：测试环境未加载插件 → 添加插件预加载
- **全面回归排查**：
  - 后端 5 个路由拆分逐行对比（git show 拆分前 → 对比拆分后），确认无遗漏
  - 插件独立化验证（BT 12 源 + 网盘 9 源注册正确）
  - 前端组件拆分验证（props 传递完整、API 路径对齐、零类型错误）
  - 33 个端点端到端测试（31 通过，2 个预期失败）
- **端到端测试报告**：`.kiro/docs/e2e-test-report.md`（用 sandbox_real 模拟完整用户流程）
- **UX 改进 TODO**：`.kiro/docs/ux-improvements-todo.md`（8 项待办，含插件分层、设置移位、导航优化）
- **文档状态更新**：
  - `new-user-experience-todo.md` 标记为 `[当前]`（P0/P1 已完成，P2 合并到 ux-improvements）
  - `codebase-cleanup-todo.md` 更新 Phase B/C/E 为已完成，Phase D 暂缓
  - `split-regression-audit-todo.md` 全部排查项标记完成

**踩坑**:
- `ShadowNameRequest` 缺失是更早的拆分（从 tools.py 移出影子名端点到 scrape.py）引入的，不是本轮路由拆分
- `clean_from_filename` 的第二个参数是 `folder_name` 不是 `source`，调用时用了错误的关键字参数
- 测试文件引用了插件化后已移除的 `shared.py` getter 函数，需要同步更新测试

**决策**:
- 前端文件拆分暂缓（风险高收益低，之前拆分已经引入了多个 bug）
- Phase D（Provider 框架移入 core/）暂缓（大规模 import 修改，当前能正常工作）
- 下一步聚焦 UX 改进（设置移位、扫描居中、插件分层、侧边栏导航）
...
**变更**:
- 新增 `plugin-center-todo.md`：完整的插件中心设计（Core/插件边界、manifest 结构、安装卸载 API、前端插件中心页面、降级体验）
- 新增 `new-user-experience-todo.md`：新用户体验优化规划（首次启动引导、媒体库结构灵活化、空状态引导、术语优化、设置页简化）

**决策**:
- 公开版默认只有 Core 功能（扫描/浏览/整理/质量分析/AI 框架），所有外部连接能力以插件形式存在
- 插件分 5 类：元数据源 / 搜索源 / RSS 订阅源 / 下载器 / 增强功能
- 侧边栏底部固定"🧩 插件中心"入口，提供安装/卸载/配置统一管理
- 私有自用版 `installed_plugins` 默认全装，行为等价当前
- 去除一级分类文件夹硬性假设，支持平铺结构和自动检测
- 术语面向普通用户优化（刮削→获取影视信息、NFO 不暴露、影子名→英文别名）

**执行顺序**:
- 插件中心先行（后端框架 → 功能拆分 → 前端页面 → 降级体验）
- 新用户体验跟进（Setup Wizard → 结构灵活化 → 空状态引导 → 文案优化）
...
## 2026-05-21 新用户体验 P1 全部收口

**变更**:

### P1 空状态引导
- 搜索弹窗：无搜索源插件时显示"未安装搜索源插件" + 引导去插件中心
- 发现页：无 discover 插件时在发现区域位置显示引导卡片
- 订阅页：无 subscribe 插件时显示引导，有插件无订阅时提示"在推荐或探索中点击订阅开始追更"
- 下载管理：已有（之前实现）

### P1 术语文案优化
- "刮削" → "识别"/"自动识别"（按钮、进度提示、状态 title、步骤标签）
- "旧刮削清理" → "旧数据清理"
- "刮削确权" → "识别匹配"
- "刮削缓存" → "识别缓存"
- "刮削候选匹配" → "识别候选匹配"
...
### P1 设置页简化
- 高级设置折叠：播放器路径、缓存管理、标准化名称管理、回收站收入可折叠"高级设置"区域，默认收起
- 连通性测试按钮：TMDB Key / Prowlarr / qBittorrent / OpenList 各字段旁加"测试"按钮，实时验证连接状态
- 修改配置值后自动重置测试状态

**决策**:
- "Provider" 不改：用户看到的已是"数据源"/"搜索源"等友好名称，Provider 只在代码内部使用
- "Action Plan" / "dry-run" 不改：用户看到的已是"整理方案"/"整理替换预览"
- "not_found" / "scrape_failed" 不改文案：用户看到的是图标（⚠/✗），title 已改为"识别失败"
- 排除文件夹保留在基础区域（不折叠）：新用户添加路径时可能需要排除 @eaDir 等

**测试**: 前端构建通过，getDiagnostics 零错误

## 2026-05-25 插件生态 Phase 2b/2c + 独立插件拆分

### 变更
- Phase 2b：7 个插件迁出到社区仓库（search-prowlarr/rss-anime/rss-tv-movie/metadata-douban/download-openlist/storage-openlist/feature-subscribe）
- Phase 2c：插件自加载改造，消除兼容层
  - 12 个 bt_scraper_*.py 移入 plugins/search-bt-direct/sources/
  - 9 个 pan_scraper_*.py 移入 plugins/search-pan/sources/
  - 8 个 rss_source_*.py 移入 plugins/rss-anime/sources/ + plugins/rss-tv-movie/sources/
  - shared.py 删除 12 个 _get_*_scraper() getter
  - bt_search_provider_factory / pan_search_service / rss_provider_factory 改为从 plugin registry 获取
- 独立插件拆分：search-bt-direct 拆为 12 个独立 BT 源插件，search-pan 拆为 9 个独立网盘源插件
- 预装策略确定：Core + 11 个预装插件（TMDB/Bangumi/豆瓣/Prowlarr/Nyaa/YTS/EZTV/qB/完整性/发现/本地感知）
- plugin_guard.py 支持独立插件 ID 映射（BT_SOURCE_PLUGIN_MAP / PAN_SOURCE_PLUGIN_MAP）

### 修复的严重 bug
- main.py 启动时未调用 load_installed_plugins()（plugin registry 始终为空，搜索源不可用）
- config_manager.py load_library() 中 lib_path → self.lib_path（导致 /library/tree 500）
- library_tree.py 一级分类目录 folder_type 手动覆盖不生效（拆分时引入）
- pan_search_provider_factory.py 裸 import 已移走的模块（崩溃）
- media_library.json 文件损坏（末尾多余内容，并发写入导致）
- CardGrid 展开面板位置偏移（"添加媒体文件夹"按钮 data-card 属性干扰 DOM 索引）

### 踩坑
- 插件 __init__.py 的 register(ctx) 必须在启动时被调用，否则 _plugin_providers 为空
- 拆分文件时 __file__ 路径变化导致 os.path.dirname 计算结果不同
- conftest.py 需要把所有插件 sources/ 目录加入 sys.path，否则测试 import 失败
- fallback 代码引用已删除的模块/函数是死代码，应清理而非保留

### 待排查
- 其余 4 个后端路由拆分（media_info/scrape/search/relocate）
- 前端组件拆分（ExpandPanel/cardGridUtils/AISettingsSection）
- 详见 docs/split-regression-audit-todo.md
