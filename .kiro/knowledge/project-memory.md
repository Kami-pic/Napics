...
- 阶段 3+4 已完成：订阅 CRUD + RSS 框架(Prowlarr源) + 匹配引擎 + 定时调度 + 频率衰减 + 100分制评分 + 洗版匹配 + 前端订阅tab/按钮/角标/资源列表/日历API + 媒体库联动 + 搜索结果quality_score + 日历自动触发搜索 + 下载完成订阅回调 + 前端日历视图
- 剩余尾巴（低优先级）：千年女优 bug、新源接入（Mikan/Nyaa/人人影视）
...
- 子阶段 C 收尾：
  - `FoundResourcesList.tsx`：资源列表组件（标题+质量+大小+做种数+下载按钮）
  - SubscribePanel 搜索状态展示（last_search/search_count）
