...
1. **NFO 读取**（最可靠）：`read_video_nfo` → `season_number` + `episode_number`
   - **季号覆盖**：目录名有明确季号时始终覆盖 NFO 的季号（NFO 可能因刮削错误导致季号不对）
2. **文件名正则**（回退）：
   - `S01E02` / `S1E10` / `Season 1 Episode 2`
   - `第3季第7集` / `第1季E02` / `第2季02集`
   - `E02` / `EP12` / `第5集` / `02集`（E/EP 限制最多 3 位数字，排除 CRC32 校验码）
   - `Unnatural-02`（连字符分隔）
   - `高清02.rmvb`（中文字符后紧跟数字）
   - `03.mkv`（独立数字）
   - 中文数字：`第一集` ~ `第九十九集`
3. **季号推断**：文件名无季号时从目录名提取（`Season 1` / `S02` / `第二季`）
4. **绝对集数重映射**：本地集号全在 S1 且最大集号超过 S1 集数时，自动用 `build_absolute_episode_map` 重映射到各季
...
| 中文字符后数字 | 支持 `高清02.rmvb`、`铳墓02.rmvb` 格式 |
| CRC32 校验码 | `[E0412FB5]` 不会被误匹配为集号（E/EP 限 3 位数字） |
| NFO 季号错误 | 目录名有明确季号时覆盖 NFO 的季号 |
| 多版本同一集 | 按集号去重，不重复计数 |
| 全集平铺无季目录 | 自动检测绝对集数并重映射到各季 |
| season 节点 | 只显示当前季的完整度（seasonFilter 过滤） |
