...
│   └── _one-off/             # [一次性] 已完成归档的临时报告/调研/会话总结
│
...
- 每个文件开头用标签标注状态：
  ...
  - `[一次性]` — 已完成归档、后续不再维护的临时调试记录、报告、会话总结等
- 仍在指导当前阶段的审计、计划、边界设计文档应放在 `docs/` 根目录并标记 `[当前]`，不要放进 `_one-off/`
- 只有文档完成使命、仅作历史追溯时，才移动到 `_one-off/` 并改为 `[一次性]`
