...
元数据源、搜索、下载、订阅、播放、字幕都做成了插件，按需装、按需卸。

镜像内置这些（装完即用，可在插件中心关掉）：

| 插件 | 做什么 |
|---|---|
| TMDB / 豆瓣 / Bangumi | 元数据刮削三源 |
| Prowlarr | BT 索引器聚合搜索 |
| qBittorrent | BT 下载 |
| OpenList 浏览 | 网盘挂载浏览与转存 |
| Web 播放器 | 浏览器内播放、转封装、内嵌字幕提取、多音轨 |
| 字幕搜索与下载 | 三源搜字幕（assrt / SubHD / SubDL），按剧集匹配后一键下载 |
| 季集完整性检测 | 对比 TMDB 找出缺哪几集 |
| 发现推荐 | 基于媒体库偏好的个性化推荐 |
| 订阅追更 | RSS 追更 + 自动洗版 |
| 本地媒体感知 | 搜索与推荐结果标注「本地已有」 |

具体的资源站搜索源、RSS 源包由第三方插件源提供，从插件中心自行安装，
不随镜像分发（见下方「法律声明」）。
...
镜像只携带上面列出的核心插件。搜索源、RSS 源这类涉及具体站点的实现
不打进镜像，安装后放在 `/app/data/plugins`，所以更新镜像不会丢已装插件。
...
- Python 3.10+
- Node.js 20+ —— Next.js 16 的最低要求，Node 18 装不上也构建不过
...
访问 `http://localhost:3032`（前端端口，后端 `8001` 由前端代理转发，不用直接访问）

手机浏览器打开 `http://<这台机器的局域网IP>:3032/m` 是移动版，详见下面「移动版」。

> 上面的后端启动命令用了 `--host 0.0.0.0`，方便你从局域网其他设备调试。
> 后端没有任何认证，`8001` 一旦可达就等于把文件操作接口开放出去了。
> 只在本机用的话把它改成 `--host 127.0.0.1` 更稳妥。Docker 镜像里已经是这样。

### 移动版

手机和平板访问 `/m`，是为触屏单独写的一套界面：媒体库浏览、原生播放、
发现榜单、搜索下载、任务进度。整理、批处理、洗版这类操作只在桌面端提供 ——
手机上误触的代价太高。桌面端在窄屏下会提示可以切到移动版，**不做 UA 自动跳转**。

移动版只需要开放前端一个端口，后端 `8001` 不必对局域网可达。

#### 局域网安全边界

移动版让**同网段的任何设备**都能打开这个界面，请先明确下面几条：

- **默认没有任何认证。** 设置里可以开一个访问密码（`/auth/*`，默认关闭），
  开了之后所有页面和接口都要先登录，token 存 cookie。
  它的目的是阻止同一局域网内的其他设备误入，**不是**为暴露到公网准备的。
- 敞口最大的两个接口，移动版都会用到，**它们的可读范围都不止媒体库**：
  - `/fs/list`（选保存路径用）能列出服务端的盘符和任意目录。它**故意不受**媒体库白名单
    约束 —— 添加媒体库时你正需要浏览白名单之外的目录。只返回目录不返回文件。
  - `/playback/stream`（播放用）按路径返回文件内容。放行范围是媒体库路径加上下载目录、
    回收站和程序数据目录（`downloads/` 沙盒与整理快照都在里面），**并不限定视频扩展名**。

  只要界面可达，这两个就都可调用。这也是下面「只开放前端端口」不是建议而是要求的原因。
- **只开放前端端口。** 后端 `--host 127.0.0.1`，浏览器的请求由前端在同机转发
  （`/backend/*`）。这样局域网里只有一个端口暴露，Docker 镜像默认就是这个形态。
- 需要在外网访问，用 Tailscale / WireGuard 这类私有网络，
  不要做端口映射或公网反代。访问密码挡不住有针对性的攻击。
- 开了访问密码后，**部署形态必须与后端同源**（默认就是）。跨源部署时
  `<video>` 带不了自定义请求头，播放会因为拿不到凭据而 401。
...
- 装在 NAS 上的 qBittorrent / Prowlarr / OpenList 可以直接用 `http://127.0.0.1:端口` 访问，
  不需要额外配置
- 镜像里后端只监听 `127.0.0.1:8001`，局域网其他设备访问不到，安全性不降低
...
桥接模式必须显式给 DNS（`--dns`），否则容器内域名全部解析不了。
媒体目录仍建议容器内外用同一个路径，理由见下方要点 1。
...
`.env` 里只有 `MEDIA_PATH` 必填，其余都有默认值：

| 变量 | 默认 | 说明 |
|---|---|---|
| `MEDIA_PATH` | 无，必填 | 媒体目录。容器内外同路径，设置页直接填它 |
| `FRONTEND_PORT` | `3032` | 宿主机端口，冲突时改这个 |
| `NAPICS_TAG` | `latest` | `latest` 是 release 分支，想用开发版填 `dev` |
| `DNS1` / `DNS2` | `223.5.5.5` / `119.29.29.29` | compose 走桥接网络，DNS 必须显式给 |
| `TZ` | `Asia/Shanghai` | 时区 |

飞牛 OS 的媒体目录一般在 `/vol1/1000/` 或 `/vol2/1000/` 下，
群晖是 `/volume1/`，威联通是 `/share/`。

compose 用的是桥接网络加显式 DNS，不是 host 模式。已经预置了
`host.docker.internal`，填 NAS 上其他服务的地址时可以直接用。
...
设置页里填的路径是给容器看的，不是给 NAS 看的。所以上面的示例都建议
**容器内外挂载成同一个路径**（`-v /vol1/1000/视频:/vol1/1000/视频`），
这样设置页直接填 `/vol1/1000/视频/电影`，不用在两套路径之间做心算转换，
下载器返回的路径也能直接对上。

如果你把它映射成了别的路径（比如 `-v /vol1/1000/视频:/media`），
那设置页就必须填 `/media/电影`，填 NAS 上的原始路径会提示路径不存在。
...
容器内的 `127.0.0.1` 指向容器自己，所以这些服务要填 `http://host.docker.internal:端口`
这种形式。用图形界面创建容器时，如果没有「添加主机」选项，改填 NAS 的局域网 IP 也可以。

**端口要填它映射到宿主机的那个，不是容器内的端口。** 这两个经常不一样：
qBittorrent 容器内默认监听 8080，但如果宿主机 8080 已被别的容器占用，
你当初大概会把它映射成 8085 之类。填错就是连不上。
用 `docker ps` 看 PORTS 列里箭头左边的数字，那个才是要填的。
...
2. **扫描路径** — 媒体库目录（支持网络路径）
3. **插件** — 在插件中心按需启用（Web 播放器 / 字幕搜索与下载 / qBittorrent /
   OpenList / 订阅追更等），需要填服务地址的插件在各自的配置项里填
4. **HTTP 代理** — 海外服务访问（可选）
...
- assrt 源需要自己去 [assrt.net](https://assrt.net/user/center) 申请 API Key 填到插件配置里，
  免费配额有每分钟调用次数上限，短时间内连续搜多部会被限流。
- SubHD / SubDL 不需要 Key，但站点结构变动时可能失效，属于第三方站点的正常风险。

其他：

- 单机单用户设计，没有账号体系，请勿直接暴露到公网。
  设置里可以开一个**访问密码**（默认不开），用来阻止同一局域网内的其他设备误入 ——
  它挡不住有针对性的攻击，外网访问请配 Tailscale / WireGuard 这类私有网络。
