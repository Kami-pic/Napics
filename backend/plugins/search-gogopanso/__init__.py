...
def register(ctx):
    """注册 狗狗盘搜 网盘搜索源"""
    import importlib
    mod = importlib.import_module("pan_scraper_gogopanso")
    scraper_cls = getattr(mod, "GogoPansoScraper")

    ctx.register_pan_search_provider(
        provider_id="gogopanso",
        name="狗狗盘搜",
        search_fn=None,
        enabled=True,
        description="阿里云盘资源搜索",
    )
    # 存储 scraper_class 供 pan_search_service 使用
    from plugin_context import _plugin_providers
    _plugin_providers["gogopanso"]["scraper_class"] = scraper_cls
    ctx.logger.info("狗狗盘搜 网盘搜索源已加载")
