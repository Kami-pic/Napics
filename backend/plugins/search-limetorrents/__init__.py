"""LimeTorrents BT 搜索源插件。

安装后提供：综合 BT 搜索（TLS 兼容性问题，默认禁用）
"""
...
def register(ctx):
    """注册 LimeTorrents 搜索源"""
    import importlib
    mod = importlib.import_module("bt_scraper_limetorrents")
    scraper_cls = getattr(mod, "LimeTorrentsScraper")
    ...
    ctx.register_scraper_search_provider(
        ...
        enabled=True,
        supports_proxy=True,
        description="综合 BT 搜索（TLS 兼容性问题，默认禁用）",
    )
    ctx.logger.info("LimeTorrents 搜索源已加载")
