...
def register(ctx):
    """注册 PanSearch 网盘搜索源"""
    import importlib
    mod = importlib.import_module("pan_scraper_pansearch")
    scraper_cls = getattr(mod, "PanSearchScraper")

    ctx.register_pan_search_provider(
        provider_id="pansearch",
        name="PanSearch",
        search_fn=None,
        enabled=True,
        description="网盘资源搜索（国内直连）",
    )
    # 存储 scraper_class 供 pan_search_service 使用
    from plugin_context import _plugin_providers
    _plugin_providers["pansearch"]["scraper_class"] = scraper_cls
    ctx.logger.info("PanSearch 网盘搜索源已加载")
