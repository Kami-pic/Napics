...
def register(ctx):
    """注册 YTS 搜索源"""
    import importlib
    mod = importlib.import_module("bt_scraper_yts")
    scraper_cls = getattr(mod, "YTSScraper")
    ...
    ctx.register_scraper_search_provider(
        ...
        enabled=True,
        supports_proxy=True,
        description="高清电影 BT 搜索（公开 API，需代理）",
    )
    ctx.logger.info("YTS 搜索源已加载")
