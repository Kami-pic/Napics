"""
刮削器：TV 系列刮削逻辑（从 scraper.py 拆分）
包含 _search_tmdb、_episode_nfo_matches_target、_scrape_collection、_scrape_tv_v3、_scrape_tv
"""
import os
import logging
import re
...
from nfo_handler import (
    read_nfo, read_video_nfo,
    write_tvshow_nfo, write_season_nfo, write_episode_nfo,
)
...
def _scrape_collection(folder_path, folder_name, subdirs, video_files,
    ...
    if subdirs and depth < max_depth:
        from scraper import scrape_folder
        for sub in subdirs:
