...
def test_batch_move_loose_video_syncs_sidecars_and_library(monkeypatch):
    ...
    try:
        ...
        moved_video = target_dir / "Old Name.mkv"
        assert result["failed"] == []
        assert result["success"] == [str(video)]
        assert moved_video.exists()
        assert (target_dir / "Old Name.nfo").exists()
        assert (target_dir / "Old Name-poster.jpg").exists()
