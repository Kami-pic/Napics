...
def test_shared_getters():
    """验证插件化后 BT 直搜源通过 bt_search_provider_factory 获取"""
    from bt_search_provider_factory import get_direct_bt_scraper_factories, DIRECT_BT_SOURCE_ORDER
    # 验证工厂函数存在且可调用
    assert callable(get_direct_bt_scraper_factories)
    # 验证源列表定义完整
    assert len(DIRECT_BT_SOURCE_ORDER) == 12
