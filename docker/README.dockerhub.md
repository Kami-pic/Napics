...
手机浏览器打开 `http://<NAS的IP>:3032/m` 是为触屏单独写的移动版：媒体库浏览、
原生播放、发现榜单、搜索下载、任务进度。整理和批处理只在桌面端提供。

它意味着**同网段的任何设备**都能打开这个界面，所以：

- **默认没有任何认证。** 设置里可以开访问密码（默认关闭），开了之后所有页面和
  接口都要先登录。它用来阻止局域网内其他设备误入，不是为公网暴露准备的。
- 移动版会用到两个敞口最大的接口，**可读范围都不止媒体库**：`/fs/list`（选保存路径用，
  能列出容器内任意目录，故意不受媒体库白名单约束）和 `/playback/stream`（播放用，
  放行范围含下载目录、回收站和程序数据目录，不限定视频扩展名）。界面可达就都可调用。
- 镜像里后端只监听 `127.0.0.1:8001`，浏览器请求由前端在容器内转发。
  **不要**把 8001 额外映射出去。
- 要在外网用，走 Tailscale / WireGuard 这类私有网络，不要端口映射或公网反代。
