    ...
    <aside className={`h-screen bg-[#141414] border-r border-white/[0.06] flex flex-col transition-all duration-300 ${collapsed ? "w-16" : "w-64"}`}>
      ...
      <div className="border-t border-white/[0.06] p-2 flex gap-1">
        <button onClick={onOpenPlugins}
          className={`flex-1 flex items-center gap-2 py-2.5 rounded-lg text-slate-500 hover:text-slate-300 hover:bg-white/5 transition-all ${collapsed ? "justify-center px-2" : "px-3"}`}>
          <span className="text-sm">🧩</span>
          {!collapsed && <span className="text-xs">插件</span>}
        </button>
