...
def load_config() -> Config:
    return Config(
        ...
        napics_api_base=os.getenv("NAPICS_API_BASE", "http://127.0.0.1:8001").rstrip("/"),
        napics_agent_token=os.getenv("NAPICS_AGENT_TOKEN", ""),
        napics_container=os.getenv("NAPICS_CONTAINER", "napics"),
        prowlarr_container=os.getenv("PROWLARR_CONTAINER", "prowlarr"),
        prowlarr_url=os.getenv("PROWLARR_URL", "http://127.0.0.1:9696").rstrip("/"),
        qb_url=os.getenv("QB_URL", "http://127.0.0.1:8085").rstrip("/"),
